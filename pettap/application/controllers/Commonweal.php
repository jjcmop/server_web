...
class CommonwealController extends Yaf\Controller_Abstract
{
    ...
    public function indexAction()
    {
        ...
        $picdata = array(
            array('pic' => '/rescue/bg_rescue1.png', 'url' => 'http://www.pettap.cn/wap/introduce?type=introduce'),
            array('pic' => '/rescue/bg_rescue2.png', 'url' => 'http://www.pettap.cn/wap/about?type=about'),
            array('pic' => '/rescue/bg_rescue3.png', 'url' => 'http://www.pettap.cn/wap/introduce?type=introduce'),
            array('pic' => '/rescue/bg_rescue4.png', 'url' => 'http://www.pettap.cn/wap/about?type=about'),
            array('pic' => '/rescue/bg_rescue5.png', 'url' => 'http://www.pettap.cn/wap/about?type=about')
        );
    ...
    public function detailAction()
    {
        ...
        $oneData['top_pic'] = '/rescue/bg_rescue1.png';
        //宠爱之家App获取某公益的捐款信息
        $currentPage = empty($_GET["page"]) ? "1" : $_GET["page"];
        $showPage = empty($_GET["showpage"]) ? "5" : $_GET["showpage"];
        $start = ($currentPage - 1) * $showPage;
        $sql = "select a.weal_id , a.user_id,a.user_nickname,a.user_account,a.pay_amount,a.pay_time,b.avatar from zs_weal_order a  left join zs_user b on a.user_id=b.id where a.weal_id='" . $weal_id . "' 
 ...
 zs_commonweal b on a.weal_id=b.id where a.user_id='" . $id . "'  and a.pay_status=1 and a.pay_weal_status=1  order by a.pay_time desc LIMIT {$start},{$showPage}  ";
    ...
    public function createorderAction(){
        ...
        $user_id=$request['id'];
        $user= $this->db->field("account,nick_name")->table("zs_user")->where("id = {$user_id}")->find();
        ...
        if($bool)
        {
            $aliPayString=$this->apppay($weal['remark'],$request['weal_title'],$data['order_number'],$request['pay_amount']);
