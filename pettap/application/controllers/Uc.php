...
class UcController extends Yaf\Controller_Abstract  {
    ...
    public function userinfoAction(){
        ...
        $resultdb = $this->db->field("*")->table("zs_user")->where("mobile_num = {$reqdata['phone_number']}")->find();
        if(empty($resultdb)){
    ...
    public function pzoneAction(){
        ...
        if(empty($reqdata["u_id"])){
            ...
            if(!empty($userinfo)){
                $sqlstr = "SELECT c.id as c_id,u.avatar,u.nick_name,u.competence,c.lbs,c.content,c.card_pic,c.click as click_num,c.create_time FROM zs_card as c LEFT JOIN zs_user as u ON u.id = c.u_id WHERE u.id = {$pet_id} ORDER BY c.id DESC LIMIT {$start},{$showPage}";
                $userinfocard = $this->db->action($sqlstr);
                foreach ($userinfocard as $key=>$value){
                    $userinfocard[$key]['is_click'] = !empty($this->db->field("*")->table("zs_card_click")->where(" u_id = {$pet_id} and c_id = {$value['c_id']}")->select()) ? 1 : 0;
                }
                echo json_encode($userinfocard);
            }else{
                echo json_encode(["code"=>1004,"message"=>"手机号不存在，请注册"]);
    ...
    public function searchAction(){
        ...
        $result = $this->db->field("id,account")->table("zs_user")->where("account")->like($account)->select();
