...
class AttentionController extends Yaf\Controller_Abstract{
    ...
    public function petfriendAction(){
        ...
        if(!empty($arrData)){
            ...
            $sql = "SELECT id,u_id,content,card_pic FROM zs_card WHERE u_id = {$f_id} ORDER BY id DESC LIMIT 0,3";
            $cardData = $this->db->action($sql);
            $arr['pzone'] = $cardData;
            $arr['is_attention'] = empty($this->db->field("*")->table("zs_attention")->where("uid = {$uid} and f_id = {$f_id}")->find()) ? "0":"1";
